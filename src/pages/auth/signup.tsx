...
export default function Signup() {
  ...
  const [done, setDone]         = useState(false); // kayıt tamam, e-posta gönderildi
        ...
        <div className="auth-card animate-slide-up" style={{ width: '100%', maxWidth: '440px' }}>
          <div style={{ textAlign: 'center', marginBottom: '2rem' }}>
            ...
            <h1 style={{ fontSize: '1.6rem', fontWeight: 800, color: '#111827', letterSpacing: '-0.02em', marginBottom: '0.375rem' }}>
              {done ? 'E-postanızı Doğrulayın' : 'Kayıt Ol'}
            </h1>
            <p style={{ color: '#6B7280', fontSize: '0.875rem' }}>
              {done ? 'Son bir adım kaldı' : 'Fırat İkinci El platformuna katılın'}
            </p>
          </div>
          ...
          {done ? (
            <div style={{ textAlign: 'center' }}>
              ...
              <div className="alert alert-success" style={{ textAlign: 'left', marginBottom: '1.25rem' }}>
                <strong>{email}</strong> adresine bir doğrulama e-postası gönderdik.
                Lütfen gelen kutunuzdaki (veya spam klasörünüzdeki) bağlantıya tıklayarak
                hesabınızı doğrulayın, ardından giriş yapın.
              </div>
              <Link href="/auth/signin" className="btn btn-primary btn-lg" style={{ width: '100%', justifyContent: 'center' }}>
                Giriş Sayfasına Git
              </Link>
            </div>
          ) : (
